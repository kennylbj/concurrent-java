...
public class Server implements EventHandler {
    ...
    public void handleAccept(SelectableChannel channel) {
        Preconditions.checkState(channel == acceptChannel);
        System.out.println("handle accept on channel " + channel.toString() + " accept channel " + acceptChannel.toString());
        try {
            ServerSocketChannel socket = (ServerSocketChannel) channel;
